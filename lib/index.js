...
//const URL = `http://${url}:${port}/${entryPoint}`;
const URL$1 = `http://localhost:3000/api/v1/users`;
...
const Uresolvers = {
	Query: {
		...
		userByEmail: (_, { email }) =>
			generalRequest(`${URL$1}/${email}`, 'GET'),
	},
	Mutation: {
		createUser: (_, { user }) =>
...
//const URL = `http://${url}:${port}/${entryPoint}`;
const URL$2 = `http://3.132.92.73:3030/api/v1/location`;
...
//const URL = `http://${url}:${port}/${entryPoint}`;
const URL$4 = `http://3.132.92.73:3030/api/v1/lodging`;
